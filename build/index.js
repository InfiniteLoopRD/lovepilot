...
class Love2DClient {
    ...
    async sendCommand(command, timeoutMs = 15000) {
    ...
    pump() {
        ...
        const timer = setTimeout(() => {
            ...
            if (this.inFlight && this.inFlight.id === id) {
                this.inFlight = null;
                next.reject(new Error(`Timed out after ${next.timeoutMs}ms waiting for a reply from the LÖVE game for command '${next.command.command}' (request id ${id}). Is the game still running and responsive? If it replies later, that late reply will now be safely discarded instead of being mismatched to a different command.`));
                this.pump();
            }
        }, next.timeoutMs);
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "get_objects",
                description: "Get objects from the current game scene. Omit 'id' to list every object " +
            ...
            {
                name: "get_screenshot",
                description: "Capture a screenshot of the currently running game window as a base64 PNG image",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            ...
            {
                name: "send_input",
                description: "Simulate keyboard or mouse input in the game, letting the AI actually play (move, attack, click, etc). " +
            ...
            },
            {
                ...
                description: "Subscribe to real-time game state updates. The game will push a notification " +
            ...
            {
                name: "list_lua_files",
                description: "List every .lua file in the game project. Real games are usually split across " +
            ...
            {
                name: "reload_code",
                description: "Hot-reload a Lua file from disk into the running game (default 'main.lua'). " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "get_objects": {
                const objectId = args && typeof args.id === "string" && args.id.trim()
                    ? args.id
                    : undefined;
            ...
            case "send_input": {
                if (!args || typeof args.type !== "string" || !args.type.trim()) {
